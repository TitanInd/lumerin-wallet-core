...
function createSyncer(config, eventBus, web3, queue, eventsRegistry, indexer) {
  ...
  function subscribeCoinTransactions(fromBlock, address) {
    ...
    let shouldResetTransactionStreamErrorCount = false;
    indexer
      ...
      .on('error',doAfter5Times(function (err) {
        debug(`Shall resync ${symbol} transactions on next block`)
        shallResync = true
      ...
      }, shouldResetTransactionStreamErrorCount));

    let shouldResetCoinBlockErrorCount = false;
      ...
      if (shallResync && !resyncing) {
        ...
        indexer
          ...
          .then(function (transactions) {
            ...
            shouldResetCoinBlockErrorCount = true;
          })
          .catch(doAfter5Times( (err) => {
              shallResync = true
              eventBus.emit('wallet-error', {
                inner: err,
          ...
          }, shouldResetCoinBlockErrorCount))
          .then(function () {
            resyncing = false
          })
      } else if (!resyncing) {
  ...
  function getPastCoinTransactions(fromBlock, toBlock, address) {
    ...
    return indexer
      ...
      .then(function (transactions) {
        ...
        return Promise.all(
          transactions.map(queue.addTransaction(address))
        ).then(() => toBlock)
      })
  }
  ...
  function getPastEventsWithChunks(options) {
    ...
    const getNewToBlock = function (index) {
      const istLastRequest =
        baseFromBlock + CHUNK_SIZE * (index + 1) === toBlock
      return Math.max(
        Math.min(
          baseFromBlock + CHUNK_SIZE * (index + 1) - (istLastRequest ? 0 : 1),
          toBlock
        ),
        minBlock
      )
    ...
    return pWhilst(
      ...
      function () {
        ...
        const newToBlock = getNewToBlock(chunkIndex)
        debug(
          'Retrieving from %s to %s for event %s',
          newFromBlock,
          newToBlock,
          eventName
        )
        return pTimeout(
          contract
            ...
            .then(function () {
              debug(
                'Retrieved from %s to %s for event %s',
                newFromBlock,
                newToBlock,
                eventName
              )
  ...
  function subscribeEvents(fromBlock, address) {
    eventsRegistry.getAll().forEach(function (registration) {
      ...
      const { contractAddress, abi, eventName, filter, metaParser } =
        registration(address)
  ...
  const refreshAllTransactions = (address) =>
    gotBestBlockPromise.then(() => {
      return Promise.all([
        getPastCoinTransactions(0, bestBlock, address),
        getPastEvents(0, bestBlock, address, function (syncedBlock) {
          bestBlock = syncedBlock
        }),
      ]).then(function ([syncedBlock]) {
        bestBlock = syncedBlock
...
const doAfter5Times = function(callback, resetCounter = false, n = 5) {
  let counter = 0
  return function () {
    
    if (resetCounter) {counter = 0; return;}
